export const examen2025IA = [

    {
    pregunta: "Son comportamientos que pueden o no ser saludables e influir de manera positiva o negativa en la salud del individuo, caracterizan el modo de vida de un individuo y suelen ser permanentes. Esta definición corresponde a:",
    opciones: [
    "A. Estilo de vida",
    "B. Hábito saludable",
    "C. Autocuidado",
    "D. Práctica"
    ],
    respuesta: "A. Estilo de vida",
    },
    {
    pregunta: "Los medicamentos que han perdido su eficacia y que han llevado a diezmar la efectividad en la lucha contra la tuberculosis, las enfermedades diarreicas y la neumonía debido a la resistencia antimicrobiana, se debe a:",
    opciones: [
    "A. Uso irracional de medicamentos",
    "B. Uso de medicamentos genéricos",
    "C. Petitorio de los mismos medicamentos",
    "D. Uso adecuado de medicamentos de marca"
    ],
    respuesta: "A. Uso irracional de medicamentos",
    },
    {
    pregunta: "Comprende la sucesión de eventos que ocurren a lo largo de la existencia de las personas y la población, los cuales interactúan para influir en su salud desde la preconcepción hasta la muerte. ¿A qué definición se refiere?",
    opciones: [
    "A. Curso de vida",
    "B. Cuidado integral",
    "C. Vías de cuidados",
    "D. Determinante social"
    ],
    respuesta: "A. Curso de vida",
    },
    {
    pregunta: "Se labora en un establecimiento de salud donde no hay una visión definida sobre el trabajo que se realiza; y existen muchos problemas interpersonales. Si se desea cambiar esta situación. ¿Qué se debe mejorar?",
    opciones: [
    "A. Clima organizacional",
    "B. Recursos humanos",
    "C. Plan de acción",
    "D. Proyecto de mejora"
    ],
    respuesta: "A. Clima organizacional",
    },
    {
    pregunta: "Un serumista luego de haber diagnosticado y tratado un caso clínico muy interesante, ha hecho una revisión profunda de la literatura sobre el tema y desea compartirlo con la comunidad científica. ¿Qué tipo de informe de investigación debe preparar?",
    opciones: [
    "A. Artículo original",
    "B. Revisión sistemática",
    "C. Reporte de caso",
    "D. Carta al editor"
    ],
    respuesta: "B. Revisión sistemática",
    },
    {
    pregunta: "Milena, una obstetra serumista coloca una ampolla intramuscular de metamizol a una gestante que viene con fiebre, minutos después siente mareos, nauseas y se observa en la zona de aplicación un rash; por tanto ella debe consignar en la historia clínica de manera obligatoria las sospechas de reacciones adversas y reportarlas en el formato correspondiente y remitirlo al comité de ...... y tecnovigilancia de la IPRESS en los plazos correspondientes.",
    opciones: [
    "A. auditoría interna",
    "B. auditoría externa",
    "C. control interno",
    "D. farmacovigilancia"
    ],
    respuesta: "D. farmacovigilancia",
    },
    {
    pregunta: "Al llegar a su comunidad de SERUMS observa que hay baja adherencia a la lactancia materna por lo cual plantea hacer una investigación sobre factores asociados a lactancia materna. ¿Qué debe realizar en primer lugar para su estudio?",
    opciones: [
    "A. Definir sus instrumentos de medición",
    "B. Revisar literatura científica sobre el tema",
    "C. Elaborar su pregunta de investigación",
    "D. Elegir los objetivos de estudio"
    ],
    respuesta: "B. Revisar literatura científica sobre el tema",
    },
    {
    pregunta: "Si se orienta la formulación y actualización de las políticas y los planes en todos los niveles del gobierno, se refiere a ...…, a las cuales debe alinear sus objetivos operativos.",
    opciones: [
    "A. desarrollos institucionales",
    "B. estrategias institucionales",
    "C. acciones institucionales",
    "D. organizaciones institucionales"
    ],
    respuesta: "B. estrategias institucionales",
    },
    {
    pregunta: "En el manejo integral de una paciente de 50 años con diabetes mellitus tipo II. ¿Qué valor de hemoglobina glicosilada indica un control metabólico óptimo?",
    opciones: [
    "A. < 7.5%",
    "B. < 7%",
    "C. < 6.5%",
    "D. < 6%"
    ],
    respuesta: "B. < 7%",
    },
    {
    pregunta: "Llega al centro materno una gestante proveniente del sur de nuestro país, quién solicita tener un parto vertical. Como profesional de la salud. ¿Cómo procedería para atender y responder al requerimiento de la paciente?",
    opciones: [
    "A. Persuadir su deseo",
    "B. Denegarle la atención",
    "C. Respetar su pedido",
    "D. Enviarla a otro establecimiento"
    ],
    respuesta: "C. Respetar su pedido",
    },
    {
    pregunta: "Se define como transición epidemiológica al proceso de paso del predominio de enfermedades …… a la preponderancia de las enfermedades...",
    opciones: [
    "A. degenerativas / infecciosas.",
    "B. crónicas / infecciosas.",
    "C. infecciosas / crónicas.",
    "D. infecciosas / prevalentes."
    ],
    respuesta: "C. infecciosas / crónicas.",
    },
    {
    pregunta: "Madre quechua hablante de 30 años, tiene niño de un año con diarrea y deshidratación; ella niega su atención en el establecimiento de salud, porque según su creencia el caso corresponde a “susto” y lo debe atender el curandero. ¿Cómo procedes ante este caso?",
    opciones: [
    "A. Solicitar al presidente de la comunidad que intervenga",
    "B. Llegas a un acuerdo en la terapia del niño, respetando sus creencias",
    "C. Discrepas con su decisión porque no es correcto",
    "D. Constatar con la policía y la autoridad comunal"
    ],
    respuesta: "B. Llegas a un acuerdo en la terapia del niño, respetando sus creencias",
    },
    {
    pregunta: "¿Cuál es el plazo máximo en días para entregar copia autenticada de la historia clínica y epicrisis, cuando el usuario de salud o su representante legal lo solicite?",
    opciones: [
    "A. 5",
    "B. 7",
    "C. 10",
    "D. 3"
    ],
    respuesta: "A. 5",
    },
    {
    pregunta: "Dentro del paquete preventivo de salud del adolescente. ¿Cuántas mediciones al año de hemoglobina debe realizarse?",
    opciones: [
    "A. 1",
    "B. 6",
    "C. 4",
    "D. 2"
    ],
    respuesta: "D. 2",
    },
    {
    pregunta: "El análisis de la situación actual es una fase del planeamiento estratégico. ¿En qué fase del ciclo se encuentra?",
    opciones: [
    "A. 4",
    "B. 3",
    "C. 2",
    "D. 1"
    ],
    respuesta: "D. 1",
    },
    {
    pregunta: "Un paciente fue referido para evaluación y tratamiento por una probable gastritis a un nivel de atención superior. Se resolvió su problema de salud de manera parcial, continuando su tratamiento en su establecimiento de origen. ¿En qué condición considera fue contrarreferido?",
    opciones: [
    "A. Curado",
    "B. Retiro voluntario",
    "C. Mejorado",
    "D. Deserción"
    ],
    respuesta: "C. Mejorado",
    },
    {
    pregunta: "Salud Escolar lleva a cabo la vacunación masiva, descarte de anemia, cuidado de la salud bucal, desparasitación a los alumnos de las IE públicas. En cumplimiento al derecho de ......, se hace de conocimiento a los padres de familia quienes firman y el personal interviene atendiendo.",
    opciones: [
    "A. beneficencia",
    "B. justicia",
    "C. consentimiento informado",
    "D. autorización"
    ],
    respuesta: "C. consentimiento informado",
    },
    {
    pregunta: "Luego de la recolección y procesamiento de datos de su estudio para investigar si hay asociación entre el alcoholismo y la violencia familiar en adultos varones de su comunidad. ¿Qué análisis estadístico llevará a cabo?",
    opciones: [
    "A. Inferencial",
    "B. Descriptivo",
    "C. Inductivo",
    "D. Probabilístico"
    ],
    respuesta: "A. Inferencial",
    },
    {
    pregunta: "En una institución educativa primaria de 100 alumnos, se presenta un brote de varicela con 50 casos. ¿Cuál es el indicador y el valor que corresponde para caracterizar el riesgo?",
    opciones: [
    "A. Tasa de prevalencia, 500 x 1000 hab",
    "B. Razón de prevalencia, 50%",
    "C. Tasa de incidencia, 50%",
    "D. Tasa de ataque, 50%"
    ],
    respuesta: "C. Tasa de incidencia, 50%",
    },
    {
    pregunta: "Cuenta con las herramientas administrativas para garantizar que la profesión se ejerza de manera ética. Este enunciado pertenece a la:",
    opciones: [
    "A. Deontología",
    "B. Responsabilidad",
    "C. Comisión de ética",
    "D. Moral"
    ],
    respuesta: "A. Deontología",
    },
    
    {
    pregunta: "¿Cuál es la sospecha diagnóstica de cáncer infantil más probable cuando hay un aumento no controlado de glóbulos blancos en la médula ósea?",
    opciones: [
    "A. Neuroblastoma",
    "B. Linfoma",
    "C. Leucemia",
    "D. Tumor de Wilms"
    ],
    respuesta: "C. Leucemia",
    },
    {
    pregunta: "Los sistemas de vigilancia en salud (SVES) son la guía para la acción que el personal de salud utiliza para tomar decisiones prácticas frente a los problemas de salud. El tipo tradicional se caracteriza por que se...",
    opciones: [
    "A. usan datos específicos de información adicional.",
    "B. usa mediciones sistemáticas.",
    "C. incluye los procedimientos del proceso de la práctica «centinela».",
    "D. basan en los sistemas de declaración de enfermedades transmisibles y morbilidad."
    ],
    respuesta: "D. basan en los sistemas de declaración de enfermedades transmisibles y morbilidad.",
    },
    {
    pregunta: "Un profesional de la salud desea realizar una ecografía, pero no cuenta en su centro con ese apoyo diagnóstico. Para realizar una referencia a otro establecimiento de salud. ¿Qué debe evaluar?",
    opciones: [
    "A. Categorización",
    "B. Organigrama",
    "C. Flujograma",
    "D. Cartera de servicios"
    ],
    respuesta: "D. Cartera de servicios",
    },
    {
    pregunta: "Para investigar la relación entre la anemia y el peso de niños de 6° grado de primaria de un colegio, usted acude un día de la semana para medir la Hb y el peso a todos ellos. ¿Qué método de investigación está realizando?",
    opciones: [
    "A. Mixto",
    "B. Experimental",
    "C. Observacional",
    "D. Cuasi experimental"
    ],
    respuesta: "C. Observacional",
    },
    {
    pregunta: "El lunes 30 de marzo se apersonan a un puesto de salud de categoría I-3, 15 personas con vómitos y diarrea, quienes participaron en una pollada el día anterior. El caso es identificado como una Enfermedad Transmitida por Alimentos. ¿En qué momento se realiza la notificación epidemiológica del brote al nivel inmediato superior?",
    opciones: [
    "A. De manera inmediata",
    "B. Cuando se reporta el VEA de la semana",
    "C. Al alta de los pacientes",
    "D. Hasta las 48 horas"
    ],
    respuesta: "A. De manera inmediata",
    },
    {
    pregunta: "Para elaborar los planos de un futuro establecimiento de salud I-3, se reúne el equipo de salud. ¿Qué ambiente es necesario según normativa?",
    opciones: [
    "A. Zona de selección de residuos sólidos",
    "B. Depósito y lavado de carros",
    "C. Zona de almacenamiento de residuos contaminados",
    "D. Área de internamiento"
    ],
    respuesta: "D. Área de internamiento",
    },
    {
    pregunta: "Después de brindar una atención por violencia sexual a una paciente de 13 años, se debe presentar un informe social, requerido por las autoridades del Poder Judicial. Si hay información que faltan a la verdad está cometiendo actos en contra de:",
    opciones: [
    "A. Justicia",
    "B. Ética",
    "C. Responsabilidad",
    "D. Beneficencia"
    ],
    respuesta: "A. Justicia",
    },
    {
    pregunta: "Usted se entera que en un EESS administraron una medicación que produjo un daño discapacitante en un paciente. ¿Qué medida inmediata hubiera tomado para manejar esta situación?",
    opciones: [
    "A. Informar a los familiares",
    "B. Mejorar el proceso de atención sanitaria en EESS",
    "C. Notificarlo al nivel inmediato superior",
    "D. Medidas de mejora en el EESS"
    ],
    respuesta: "C. Notificarlo al nivel inmediato superior",
    },
    {
    pregunta: "Mujer de 52 años acude al establecimiento refiriendo sequedad vaginal y dispareunia desde hace unos 6 meses, tiene antecedente de menopausia hace un año. ¿Cuál es la conducta más apropiada para esta paciente?",
    opciones: [
    "A. Prescribir terapia de reemplazo hormonal",
    "B. Recomendar uso de lubricante vaginal",
    "C. Referir a especialista de ginecología",
    "D. Prescribir estrógeno local vaginal"
    ],
    respuesta: "D. Prescribir estrógeno local vaginal",
    },
    {
    pregunta: "¿Cuáles son los medicamentos y productos biológicos que cubren la mayor parte de la morbilidad en el país, que luego de una evaluación técnica especializada multidisciplinaria han demostrado ser seguros, eficaces, costo-efectivos y deben estar disponibles en todo momento?",
    opciones: [
    "A. Genéricos",
    "B. Comunes",
    "C. Básicos",
    "D. Esenciales"
    ],
    respuesta: "D. Esenciales",
    },
    {
    pregunta: "Usted trabaja en un centro de salud y advierte que la población que acude son personas adultas mayores en un 80%. ¿Qué medida inicial puede tomar para mejorar su atención?",
    opciones: [
    "A. Tener un enfoque de derechos",
    "B. Favorecer la participación de la familia",
    "C. Promover la educación en la comunidad",
    "D. Fortalecer su equipo multidisciplinario"
    ],
    respuesta: "D. Fortalecer su equipo multidisciplinario",
    },
    {
    pregunta: "¿Quién es el responsable frente a la pérdida de la totalidad o parte de la historia clínica en el proceso de atención de salud?",
    opciones: [
    "A. Quien la entregó por última vez",
    "B. Jefe de archivo de historias clínicas",
    "C. Jefe del establecimiento de salud",
    "D. Quien la recibió por última vez"
    ],
    respuesta: "D. Quien la recibió por última vez",
    },
    {
    pregunta: "La salud pública está afectada por sobrepoblación mundial, grandes conflictos políticos-sociales y la pobreza creciente; como consecuencia inicial de estos fenómenos. ¿Qué ocurre principalmente?",
    opciones: [
    "A. La aparición de enfermedades epidémicas",
    "B. El surgimiento de nuevas enfermedades",
    "C. El aumento en la incidencia de enfermedades",
    "D. Las crisis psicosociales"
    ],
    respuesta: "C. El aumento en la incidencia de enfermedades",
    },
    {
    pregunta: "Para medir el riesgo de una enfermedad en la comunidad, se utilizan medidas de morbilidad como la incidencia que mide la …... y la prevalencia que mide la …… de la enfermedad.",
    opciones: [
    "A. virulencia / frecuencia",
    "B. proporción / frecuencia",
    "C. velocidad / magnitud",
    "D. casos antiguos / casos nuevos"
    ],
    respuesta: "C. velocidad / magnitud",
    },
    {
    pregunta: "Debido a que se observaron muchos casos de suicidios en adolescentes en la comunidad donde usted labora, le encargan realizar un estudio descriptivo sobre la depresión en adolescentes en su comunidad ¿Cuál será la principal limitante del estudio?",
    opciones: [
    "A. Sus resultados no serán extrapolables",
    "B. Demandará costos económicos muy elevados",
    "C. El análisis estadístico será muy complejo",
    "D. No permitirá hallar asociación de variables"
    ],
    respuesta: "D. No permitirá hallar asociación de variables",
    },
    {
    pregunta: "El Ministerio de Salud a través de ...... es responsable de distribuir, difundir, dirigir, supervisar, monitorear y evaluar los medicamentos en los sectores públicos y privados.",
    opciones: [
    "A. DIGEMID",
    "B. CENARES",
    "C. DIGESA",
    "D. CENSOPAS"
    ],
    respuesta: "A. DIGEMID",
    },
    {
    pregunta: "Mujer de 55 años, señala que su esposo se hizo prueba de papilomavirus y salió positiva. Acude a control al centro materno y le realizan tamizaje con PAP que salió positivo. ¿Qué prueba está en el siguiente paso en el flujograma de atención de cáncer de cuello uterino?",
    opciones: [
    "A. Curetaje endocervical",
    "B. Colposcopía",
    "C. Nuevo PAP",
    "D. Tomar biopsias"
    ],
    respuesta: "B. Colposcopía",
    },
    {
    pregunta: "Un serumista está trabajando en un establecimiento de salud con población asignada, que cuenta con médico cirujano, profesionales de enfermería, obstetricia y personal técnico de enfermería. Según su cartera de servicios. ¿Cuál es el nivel de atención en el que se encuentra?",
    opciones: [
    "A. I-3",
    "B. I-1",
    "C. I-2",
    "D. I-4"
    ],
    respuesta: "C. I-2",
    },
    {
    pregunta: "El C.S. Paz, ha sectorizado su jurisdicción para realizar intervenciones de promoción y prevención basados en el Modelo de Cuidado Integral de la Familia y Comunidad por Curso de Vida. En el marco de las Funciones Esenciales de la Salud Pública renovada, esta acción corresponde a la etapa de:",
    opciones: [
    "A. Evaluación",
    "B. Acceso",
    "C. Asignación de recursos",
    "D. Desarrollo de políticas"
    ],
    respuesta: "B. Acceso",
    },
    {
    pregunta: "En el proceso de redacción del informe final de su estudio transversal analítico sobre la relación entre la ingesta de bebidas gaseosas y sobrepeso; usted desea que todas las secciones tengan coherencia entre sí. ¿Qué herramienta debe usar para verificarla?",
    opciones: [
    "A. Matriz de consistencia",
    "B. Lista de verificación",
    "C. Revisión de Turniting",
    "D. Matriz de operacionalización"
    ],
    respuesta: "A. Matriz de consistencia",
    },
    {
    pregunta: "La promoción de la salud se define como la acción de ofrecer a los pueblos las vías y los medios suficientes para …… los problemas de salud y autocontrolar el proceso de salud-enfermedad.",
    opciones: [
    "A. restringir o acortar",
    "B. atenuar o eliminar",
    "C. controlar o mantener",
    "D. diferir o mitigar"
    ],
    respuesta: "B. atenuar o eliminar",
    },
    {
    pregunta: "El POI Multianual del Ministerio de Salud es ……, en el cual se establece la programación de metas físicas y de costeo que se espera alcanzar en los próximos tres años.",
    opciones: [
    "A. herramienta de gestión",
    "B. instrumento de articulación",
    "C. herramienta de articulación",
    "D. instrumento de gestión"
    ],
    respuesta: "D. instrumento de gestión",
    },
    {
    pregunta: "Mujer de 70 años, procedente de Loreto, hace 2 días presentó fiebre de 39°C, dolor de articulaciones y cefalea, el día de hoy acude al establecimiento con vómitos persistentes y sangrado de mucosas. Funciones vitales: T°: 36°C, PA: 100/70 mmHg, FR: 28X'. ¿Qué tipo de dengue es?",
    opciones: [
    "A. Hemorrágico con signos de alarma",
    "B. Grave",
    "C. Confirmado sin signos de alarma",
    "D. Probable con signos de alarma"
    ],
    respuesta: "D. Probable con signos de alarma",
    },
    {
    pregunta: "Usted va a realizar un estudio sobre la obesidad infantil en un colegio de la pequeña comunidad donde labora, pero el director le pide que los padres autoricen voluntariamente la participación de sus menores hijos. ¿Qué documento debe usar para este cometido?",
    opciones: [
    "A. Consentimiento informado",
    "B. Compromiso paterno",
    "C. Carta de investigación",
    "D. Asentimiento informado"
    ],
    respuesta: "A. Consentimiento informado",
    },
    {
    pregunta: "¿Cuál es la mejor conducta frente a un personal de salud asistencial que no puede acreditar su vacunación previa contra el virus de Hepatitis B en el primer nivel de atención?",
    opciones: [
    "A. Vacunación de una dosis de refuerzo",
    "B. No es obligatoria su vacunación",
    "C. Referencia a un hospital",
    "D. Vacunación desde la primera dosis"
    ],
    respuesta: "D. Vacunación desde la primera dosis",
    },
    {
    pregunta: "¿Cuáles son los principales ejes estratégicos que garantizan el éxito de la participación comunitaria en la planeación en salud?",
    opciones: [
    "A. El abordaje territorial, espacios participativos y la familia como base",
    "B. Los cambios en la realidad económico-social y ordenanzas municipales",
    "C. La actitud política y el esfuerzo de la administración local",
    "D. La participación comunitaria y escuelas saludables"
    ],
    respuesta: "A. El abordaje territorial, espacios participativos y la familia como base",
    },
    {
    pregunta: "En una farmacia se va a instalar el control termostático de la unidad de refrigeración. ¿Qué se debe tener en cuenta con los sensores?",
    opciones: [
    "A. Dependientes del sistema de monitoreo",
    "B. Con exactitud de aproximadamente 0.3°C",
    "C. Deben estar calibrados",
    "D. Áreas con poca variabilidad en temperatura"
    ],
    respuesta: "C. Deben estar calibrados",
    },
    {
    pregunta: "La ética en la función pública no se queda en la teoría (reflexión), sino que debe llevarse a la práctica para tener un adecuado desempeño laboral. Como profesional de salud ante un caso de un diagnóstico que es confidencial, por el tipo de enfermedad. ¿Qué principios éticos debe cumplirse?",
    opciones: [
    "A. Silencio, manejo de la información",
    "B. Veracidad, lealtad, eficiencia",
    "C. Respeto y atención diferenciada",
    "D. Respeto, probidad y eficiencia"
    ],
    respuesta: "D. Respeto, probidad y eficiencia",
    },
    {
    pregunta: "En el análisis FODA, los aspectos positivos de los recursos y actividades internas con que cuenta la institución, se les define cómo:",
    opciones: [
    "A. Estrategias",
    "B. Fortalezas",
    "C. Oportunidades",
    "D. Amenazas"
    ],
    respuesta: "B. Fortalezas",
    },
    {
    pregunta: "¿Qué grupo de pacientes tienen indicación de terapia preventiva para la tuberculosis?",
    opciones: [
    "A. Sin diagnóstico de ITBL que pertenecen a grupos con corticoterapia e inmunosupresores",
    "B. Adultos que son contactos de caso índice-TB pulmonar y pacientes con VIH",
    "C. Personas que no tienen diagnóstico de infección tuberculosa latente",
    "D. Pacientes con tuberculosis sensible"
    ],
    respuesta: "B. Adultos que son contactos de caso índice-TB pulmonar y pacientes con VIH",
    },
    {
    pregunta: "¿Qué se define como la forma característica de pensar y hacer las cosas en una entidad, en base a principios, valores, creencias, conductas y normas?",
    opciones: [
    "A. Valores institucionales",
    "B. Cultura organizacional",
    "C. Principios institucionales",
    "D. Clima organizacional"
    ],
    respuesta: "B. Cultura organizacional",
    },
    {
    pregunta: "Varón de 20 años HSH, que acude a la consulta por presentar erupciones genitales y orales, además dolor ano-rectal y sangrado leve. Ha tenido dolor de cabeza, fiebre, mialgias y adenopatías cervicales hace 2 días. ¿Cuál es el tipo de diagnóstico de viruela del mono?",
    opciones: [
    "A. Confirmado",
    "B. Caso secundario",
    "C. Sospechoso",
    "D. Contacto directo"
    ],
    respuesta: "C. Sospechoso",
    },
    {
    pregunta: "En adultos de 18 años a más con HTA esencial, sin otras comorbilidades. ¿Qué grupo farmacológico se debe prescribir como primera elección?",
    opciones: [
    "A. Inhibidores de aldosterona",
    "B. Betabloqueadores",
    "C. Diuréticos de asa",
    "D. Diuréticos tiazídicos"
    ],
    respuesta: "D. Diuréticos tiazídicos",
    },
    {
    pregunta: "La transmisión del dengue a un huésped susceptible se da a través de la picadura de un zancudo hembra infectado (Aedes aegypti), estos zancudos pican preferentemente ...... y transmiten la enfermedad a varias personas durante toda su vida.",
    opciones: [
    "A. al amanecer y al atardecer",
    "B. al atardecer y durante la noche",
    "C. a medio día y sin lluvia",
    "D. solo en época de lluvias"
    ],
    respuesta: "A. al amanecer y al atardecer",
    },
    {
    pregunta: "Realizando visita domiciliaria, según el plan del cuidado integral de salud de las personas adultas mayores, se encuentra a un varón de 66 años de edad, vive solo; refiere tener “mal de ojo”, presenta “dolor de cabeza y dolor muscular”, llegará el curandero, realizará una limpia con cuy macho de color negro. Se le muestra interés e integración de los cuidados que requiere. ¿Éste enunciado se denomina?",
    opciones: [
    "A. Salud pública",
    "B. Cultura",
    "C. Población indígena",
    "D. Interculturalidad en salud"
    ],
    respuesta: "D. Interculturalidad en salud",
    },
    {
    pregunta: "Usted desea motivar a su personal de salud para realizar una investigación científica sobre la anemia infantil en su comunidad, para ello les dice que el principal propósito de la investigación científica es...",
    opciones: [
    "A. consolidar el trabajo en equipo.",
    "B. generar conocimiento científico útil.",
    "C. mejorar la gestión de la salud.",
    "D. captar inversiones en salud."
    ],
    respuesta: "B. generar conocimiento científico útil.",
    },
    {
    pregunta: "Dentro del modelo de atención integral en la etapa de vida adulto mayor. ¿Qué test de valoración de deterioro cognitivo puede aplicar en el primer nivel de atención?",
    opciones: [
    "A. De Pfeiffer",
    "B. De Yesavage",
    "C. Del reloj",
    "D. MOCA"
    ],
    respuesta: "A. De Pfeiffer",
    },
    {
    pregunta: "Dentro de las intervenciones de promoción de la salud. El programa “familia saludable” considera a la familia como la unidad de ...... de mayor eficacia.",
    opciones: [
    "A. análisis",
    "B. intervención",
    "C. soporte",
    "D. riesgo"
    ],
    respuesta: "B. intervención",
    },
    {
    pregunta: "¿Cuál es la medida de tendencia central que se interpreta como el promedio de los datos, y se construye con la suma de todos los datos observados entre el total de observaciones?",
    opciones: [
    "A. Rango",
    "B. Mediana",
    "C. Media",
    "D. Moda"
    ],
    respuesta: "C. Media",
    },
    {
    pregunta: "Se desea reorganizar el servicio de admisión y para ello es necesario separar las historias clínicas. ¿Cuántos años se debe considerar para el archivo pasivo de historias clínicas que no han sido requeridas?",
    opciones: [
    "A. Más de 15",
    "B. Más de 5",
    "C. Más de 1",
    "D. Más de 10"
    ],
    respuesta: "B. Más de 5",
    },
    {
    pregunta: "Dentro del petitorio nacional único de medicamentos esenciales para el sector salud existen consideraciones especiales de su uso, por lo cual requieren de un manejo especializado. ¿Qué tipo de medicamentos están en este rubro?",
    opciones: [
    "A. Para métodos de planificación familiar",
    "B. Controlado para infecciones intrahospitalarias",
    "C. Exclusivo para dolor oncológico",
    "D. Específico para hepatitis A"
    ],
    respuesta: "C. Exclusivo para dolor oncológico",
    },
    {
    pregunta: "En la metodología del análisis FODA, el comportamiento de la población, costumbre y hábitos negativos que son difíciles de cambiar, se identifican como:",
    opciones: [
    "A. Negatividad",
    "B. Oportunidades",
    "C. Debilidades",
    "D. Amenazas"
    ],
    respuesta: "D. Amenazas",
    },
    {
    pregunta: "Gestante de 36 años de edad, acude con cefalea intensa y visión borrosa; antecedente: HTA. Al examen: REG, BEN, BEH, LOTEP, FC: 70X'; FR: 20X'; T°: 36.8°C; PA: 170/110 mmHg; edema en miembros inferiores sin fóvea. Laboratorio: creatinina 1.2 mg/dl, plaquetas: 150,000/mm3. ¿Cuál es la conducta a seguir?",
    opciones: [
    "A. Sulfato de magnesio intravenoso",
    "B. Finalizar el embarazo",
    "C. Administrar nifedipino vía oral",
    "D. Transfusión de plaquetas"
    ],
    respuesta: "A. Sulfato de magnesio intravenoso",
    },
    {
    pregunta: "Un paciente desea realizar un reclamo por un posible maltrato de un trabajador del establecimiento de salud contra él, al no respetarse el orden de llegada para la atención. ¿Qué procedimiento corresponde?",
    opciones: [
    "A. Sugerencia",
    "B. Consejería",
    "C. Queja",
    "D. Consulta"
    ],
    respuesta: "C. Queja",
    },
    {
    pregunta: "¿A qué corresponde la siguiente definición?: “Es el conjunto de acciones que desarrolla un profesional de la salud mediante el uso de las TIC, para proporcionar a la persona usuaria de salud, consejería y asesoría con fines de promoción de la salud, prevención, recuperación o rehabilitación de las enfermedades”.",
    opciones: [
    "A. Telemonitoreo",
    "B. Teleconsulta",
    "C. Teleorientación",
    "D. Teleinterconsulta"
    ],
    respuesta: "C. Teleorientación",
    },
    {
    pregunta: "Según la normativa nacional de salud para la prevención y control de la anemia. ¿Cuál es la dosis oral diaria de hierro elemental y ácido fólico recomendada para tratamiento de una gestante con anemia ferropénica?",
    opciones: [
    "A. 120 mg y 800 ug",
    "B. 80 mg y 600 ug",
    "C. 60 mg y 400 ug",
    "D. 200 mg y 1000 ug"
    ],
    respuesta: "A. 120 mg y 800 ug",
    },
    {
    pregunta: "En el informe final de su investigación sobre ansiedad en estudiantes de secundaria, requiere compartir la descripción del instrumento de screening de ansiedad utilizado, su validez y confiabilidad. ¿En qué sección de su informe debe incluirla?",
    opciones: [
    "A. Marco teórico",
    "B. Introducción",
    "C. Metodología",
    "D. Resultados"
    ],
    respuesta: "C. Metodología",
    },
    {
    pregunta: "Al Centro de Salud de Chuquibamba de la provincia de Condesuyos, acude una gestante añosa, en periodo expulsivo, con antecedentes de preeclampsia; el parto se complica y fallecen madre y niño. Ante este evento. ¿Qué Auditoría de la Calidad de la Atención en Salud corresponde seguir?",
    opciones: [
    "A. Dirimente",
    "B. De caso",
    "C. Externa",
    "D. Regular"
    ],
    respuesta: "B. De caso",
    },
    {
    pregunta: "En su comunidad observa con mucha frecuencia casos de alcoholismo en varones y también violencia física de ellos contra sus esposas, para investigar en el menor tiempo posible, si hay asociación entre el alcoholismo y la violencia familiar. ¿Qué diseño de estudio elegirá?",
    opciones: [
    "A. Cuasi experimental",
    "B. Descriptivo",
    "C. Analítico",
    "D. Cohortes"
    ],
    respuesta: "C. Analítico",
    },
    {
    pregunta: "La ética sugiere aquello que es deseable y condena lo que no debe hacerse, mientras que la deontología cuenta con las herramientas administrativas para garantizar que la profesión se ejerza de manera ética. ¿Con qué debe contar un profesional de la salud para su ejercicio profesional?",
    opciones: [
    "A. Título profesional, colegiatura",
    "B. Diplomados",
    "C. Currículo vitae documentado",
    "D. Estudios de posgrado"
    ],
    respuesta: "A. Título profesional, colegiatura",
    },
    {
    pregunta: "Se desea identificar el ciclo vital familiar para abordar a una familia que tiene un hijo adolescente con riesgo social. ¿Cuál es la idea principal de ciclo vital familiar?",
    opciones: [
    "A. Secuencia de fases por las que atraviesa",
    "B. Herramienta de gestión para los cuidados integrales",
    "C. Conjunto de prestaciones según necesidades de salud",
    "D. Sucesión de eventos que ocurren durante su existencia"
    ],
    respuesta: "A. Secuencia de fases por las que atraviesa",
    },
    {
    pregunta: "El personal de un establecimiento de salud desea saber lo que debe desarrollar prioritariamente. ¿Qué componente debe conocer para orientar sus objetivos estratégicos?",
    opciones: [
    "A. Misión",
    "B. Visión",
    "C. Valores",
    "D. FODA"
    ],
    respuesta: "A. Misión",
    },
    {
    pregunta: "En un establecimiento de salud que tiene bajos indicadores de desempeño, se desea realizar un concepto compartido para un futuro mejor, pues cada quien tiene una idea diferente de lo que se quiere lograr. ¿Qué componente estratégico se debe plantear entre todos?",
    opciones: [
    "A. Visión",
    "B. FODA",
    "C. Misión",
    "D. Valores"
    ],
    respuesta: "A. Visión",
    },
    {
    pregunta: "La sala situacional de salud es una herramienta que ofrece evidencia sobre la eficacia de las intervenciones en salud y sobre la capacidad de los sistemas de salud para responder a las necesidades de salud de la población. ¿Quiénes la deben conocer y analizar prioritariamente?",
    opciones: [
    "A. Promotores de salud y responsable de promoción de la salud",
    "B. Todo el personal de salud y autoridades locales",
    "C. Coordinadores de red y responsable de epidemiología",
    "D. El responsable de epidemiología y jefe de establecimiento"
    ],
    respuesta: "B. Todo el personal de salud y autoridades locales",
    },
    {
    pregunta: "Niño de 3 años de edad, no tiene controles en crecimiento y desarrollo, por lo que el personal de salud acude a su casa para evaluarlo. ¿Cómo demostraría que está en riesgo nutricional?",
    opciones: [
    "A. Pérdida lenta de peso",
    "B. Curva Peso/Talla de lento crecimiento",
    "C. Acercamiento a la mediana",
    "D. Curva Peso/Talla es plana"
    ],
    respuesta: "D. Curva Peso/Talla es plana",
    },
    {
    pregunta: "La pertinencia cultural en los servicios de salud implica que durante todo el proceso de la atención se tome en cuenta las características de la población a la que se atiende, en función a su cultura y contexto social. Evaluando cuatro dimensiones: gestión para la calidad de la atención, revaloración del sistema de salud tradicional, recursos humanos que promueven la salud intercultural y la …",
    opciones: [
    "A. participación ciudadana.",
    "B. interculturalidad en salud.",
    "C. atención intercultural.",
    "D. inclusión social."
    ],
    respuesta: "A. participación ciudadana.",
    },
    {
    pregunta: "En el centro de salud, el serumista, atiende a varón de 20 años y le brinda consejería sobre salud bucal con la técnica del cepillado e hilo dental. ¿Qué desea implementar en la persona?",
    opciones: [
    "A. Cambio de dieta",
    "B. Destreza",
    "C. Motivación",
    "D. Autocuidado"
    ],
    respuesta: "D. Autocuidado",
    },
    {
    pregunta: "Mujer de 45 años, presenta desde hace más de dos semanas ánimo bajo, poca energía, disminución de la actividad, anhedonia, sueño alterado, apetito disminuido, baja autoestima, estado de ánimo variable y cefalea intensa. ¿Cuál es el diagnóstico más probable?",
    opciones: [
    "A. Trastorno bipolar",
    "B. Episodio depresivo",
    "C. Depresión mayor",
    "D. Distimia"
    ],
    respuesta: "B. Episodio depresivo",
    },
    {
    pregunta: "Según la historia natural de la enfermedad, identificar una persona con diabetes en la fase subclínica corresponde al periodo ...... y el nivel de prevención es...",
    opciones: [
    "A. patogénico / terciario.",
    "B. patogénico / secundario.",
    "C. pre patogénico / secundario.",
    "D. patogénico / primario."
    ],
    respuesta: "B. patogénico / secundario.",
    },
    {
    pregunta: "Ante la presencia de un caso probable de Tos Ferina. ¿Cuáles son las medidas a implementar?",
    opciones: [
    "A. Censo de contactos, barrido vacunal, investigación del caso índice",
    "B. Jornada de vacunación, quimioprofilaxis, búsqueda de contactos indirectos",
    "C. Seguimiento de contactos, búsqueda activa comunitaria e institucional, bloqueo vacunal",
    "D. Búsqueda de casos secundarios, barrido vacunal, inamovilidad del caso"
    ],
    respuesta: "C. Seguimiento de contactos, búsqueda activa comunitaria e institucional, bloqueo vacunal",
    },
    {
    pregunta: "En el centro de salud de Lamas, reportan que un varón de 30 años, presenta fiebre, malestar general, sangrado y shock. Se informa que han aparecido muchos más casos en estos días y se sospecha de un brote de fiebre amarilla. Siendo Ro: el número promedio de casos nuevos que genera un caso dado a lo largo de un período infeccioso. ¿Cuál es el Ro que se espera tener en brote de fiebre amarilla?",
    opciones: [
    "A. Ro > 0.5",
    "B. Ro > 1",
    "C. Ro = 0",
    "D. Ro < 1"
    ],
    respuesta: "B. Ro > 1",
    },
    {
    pregunta: "En el centro de salud, al colocar un inyectable intramuscular a un paciente, el personal se pincha el dedo pulgar. El status VIH del paciente es desconocido. ¿Qué acciones debe realizarse inmediatamente?",
    opciones: [
    "A. Realizarse serologías para VIH basal, a las 6 semanas y a los 3 meses",
    "B. Lavarse la herida con agua y tomar los antirretrovirales lo antes posible",
    "C. Se valora la severidad del accidente ocupacional",
    "D. Se debe iniciar los ARV a las 72 horas post-exposición"
    ],
    respuesta: "B. Lavarse la herida con agua y tomar los antirretrovirales lo antes posible",
    },
    {
    pregunta: "En la enfermedad de la Tuberculosis, la presencia del agente infeccioso (Mycobacterium tuberculosis), corresponde a una causa...",
    opciones: [
    "A. no necesaria pero suficiente.",
    "B. de tipo ambiental.",
    "C. necesaria y suficiente.",
    "D. necesaria pero no suficiente."
    ],
    respuesta: "D. necesaria pero no suficiente.",
    },
    {
    pregunta: "¿A qué corresponde la siguiente definición?: “Es la condición ideal en la población en la que todos tienen la oportunidad de acceder al cuidado de su salud y acceder a atenciones de salud, en el momento que lo requiera, y que no sea alterada por razones evitables y por tanto injustas”",
    opciones: [
    "A. Calidad",
    "B. Equidad en salud",
    "C. Integralidad",
    "D. Universalidad"
    ],
    respuesta: "B. Equidad en salud",
    },
    {
    pregunta: "Para investigar la relación entre la anemia y la talla de niños de 5° grado de primaria de un colegio, usted acude un día de la semana para medir la Hb y talla a todos ellos. ¿Qué enfoque de investigación está realizando?",
    opciones: [
    "A. Mixto",
    "B. Observacional",
    "C. Cualitativo",
    "D. Cuantitativo"
    ],
    respuesta: "D. Cuantitativo",
    },
    {
    pregunta: "Una prueba de screening que tiene muy baja sensibilidad, tiene mayor probabilidad de clasificar a sujetos enfermos como sanos. ¿Qué tipo de resultado es?",
    opciones: [
    "A. Sensibilidad ineficiente",
    "B. Falso positivo",
    "C. Falso negativo",
    "D. Valor predictivo positivo"
    ],
    respuesta: "C. Falso negativo",
    },
    {
    pregunta: "Las acciones para mejorar la calidad de la atención orientados a mejorar la experiencia y la confianza de las personas, las familias y las comunidades en los servicios de salud, más allá de la optimización de los procesos, se enmarca dentro de las Funciones Esenciales de Salud Pública. ¿A qué función corresponde?",
    opciones: [
    "A. Asegurar el acceso y el uso racional de medicamentos",
    "B. Acceso equitativo a servicios de salud integrales y de calidad",
    "C. Financiamiento de la salud eficiente y equitativo",
    "D. Desarrollo de recursos humanos para la salud"
    ],
    respuesta: "B. Acceso equitativo a servicios de salud integrales y de calidad",
    },
    {
    pregunta: "Paciente mujer de 50 años, afroamericana, últimamente ha presentado zumbido de oído y visión borrosa, niega antecedentes personales patológicos. Examen: LOTEP, BEG, BEN, BEH; FC: 80X'; FR: 20X'; T°: 37°C; PA: 140/90 mmHg, IMC: 32. ¿Cuál es la conducta ideal a seguir?",
    opciones: [
    "A. Iniciar tratamiento farmacológico",
    "B. Realizar una nueva toma de PA",
    "C. Confirmar presunción diagnóstica con AMPA",
    "D. Confirmar presunción diagnóstica con MAPA"
    ],
    respuesta: "D. Confirmar presunción diagnóstica con MAPA",
    },
    {
    pregunta: "Personal de serenazgo, lleva al centro de salud a un paciente adulto mayor con problemas de salud y sin familiares, presenta indigencia y sin documentos. ¿Qué derecho se le atribuye?",
    opciones: [
    "A. Acceso a servicios de salud",
    "B. Acceso a la información",
    "C. Recuperación",
    "D. Consentimiento informado"
    ],
    respuesta: "A. Acceso a servicios de salud",
    },
    {
    pregunta: "¿Cuándo indicaría usted iniciar la higiene oral para generar el hábito saludable del cepillado dental?",
    opciones: [
    "A. A partir del año de vida",
    "B. Al completar su dentadura decidua",
    "C. En cualquier momento de la vida",
    "D. Durante los primeros seis meses de vida"
    ],
    respuesta: "D. Durante los primeros seis meses de vida",
    },
    {
    pregunta: "En un estudio sobre la depresión en adolescentes en su comunidad, aplica un instrumento de detección de depresión en estudiantes de 5° año de secundaria de un colegio cercano al establecimiento. ¿Qué tipo de muestreo está realizando?",
    opciones: [
    "A. Estadístico",
    "B. Censal",
    "C. Probabilístico",
    "D. No probabilístico"
    ],
    respuesta: "D. No probabilístico",
    },
    {
    pregunta: "La Ética cultiva y promueve el ejercicio de la moral y los valores para que los actos estén encuadrados en el bien y en el derecho en beneficio de la sociedad. Como profesional de la salud. ¿Qué principios se debe de cumplir en la atención a la población objetivo?",
    opciones: [
    "A. Solidaridad, orden, deber y derecho",
    "B. Orden, moral, deber y derecho",
    "C. Fraternidad, moral, deber y orden",
    "D. Orden, fraternidad, moral y derecho"
    ],
    respuesta: "B. Orden, moral, deber y derecho",
    },
    {
    pregunta: "Se reportan 2 casos de peste en el hospital y se ha restringido el ingreso. Los casos son de la comunidad y no se sabe si hay más casos. ¿Cuál es la medida inicial a tomar?",
    opciones: [
    "A. Control de roedores",
    "B. Medidas de bioseguridad e identificar las casas de infectados",
    "C. Medidas de control químico de pulgas",
    "D. Ordenamiento ambiental"
    ],
    respuesta: "B. Medidas de bioseguridad e identificar las casas de infectados",
    },
    {
    pregunta: "En pro de la salud y el desarrollo social entre los diferentes actores, en los distintos niveles de gobierno y de la sociedad, lo primero que se debe fortalecer son las...",
    opciones: [
    "A. decisiones multisectoriales.",
    "B. capacidades de gestión.",
    "C. coordinaciones intersectoriales.",
    "D. alianzas estratégicas."
    ],
    respuesta: "D. alianzas estratégicas.",
    },
    {
    pregunta: "En relación al almacenamiento de medicamentos: “Las condiciones de almacenamiento deben ser las recomendadas por el …… y autorizadas en el rotulado del producto, pudiendo ser de congelación, temperatura de refrigeración, temperatura ambiente y temperatura ambiente controlada, lo cual debe estar indicado en su procedimiento …… de almacenamiento.”",
    opciones: [
    "A. fabricante / operativo",
    "B. farmacéutico / rutinario",
    "C. director / organizativo",
    "D. jefe / establecido"
    ],
    respuesta: "A. fabricante / operativo",
    },
    {
    pregunta: "Un servidor público, brinda información de un diagnóstico médico, a personas de la comunidad. ¿Qué principio ético está vulnerando?",
    opciones: [
    "A. Individualidad",
    "B. Respeto",
    "C. Seguridad",
    "D. Confidencialidad"
    ],
    respuesta: "D. Confidencialidad",
    },
    {
    pregunta: "Se requiere implementar un plan de cuidado integral de salud del adulto mayor para la prevención de caídas. ¿Qué acción de cuidados será recomendable implementar en adultos mayores frágiles?",
    opciones: [
    "A. Completar los esquemas de vacunación",
    "B. Consejería para realizar ejercicio físico",
    "C. Descarte de enfermedades crónicas",
    "D. Implementar visitas domiciliarias periódicas"
    ],
    respuesta: "B. Consejería para realizar ejercicio físico",
    },
    {
    pregunta: "Mujer de 22 años víctima de violencia física, es derivada por la fiscalía para su atención integral en el establecimiento de salud. El área de psicología luego de la evaluación, inicia su plan de atención integral que constará de 10 sesiones. ¿Qué tipo de determinante social de la salud se está abordando?",
    opciones: [
    "A. Estructural",
    "B. Intermedio",
    "C. Comercial",
    "D. Económico"
    ],
    respuesta: "A. Estructural",
    },
    {
    pregunta: "Varón de 20 años, señala haber tenido RS con pareja desconocida hace un mes y no acude al EESS para ser evaluado. Trae resultado de VDRL: 16 diluciones. FTA-ABS (+). Examen físico: PA: 110/70 mmHg, FC: 85 X', FR: 14X', afebril; en REG, con úlcera genital, no presenta ronchas en manos y plantas de pies. ¿Cuál es el diagnóstico más probable?",
    opciones: [
    "A. Gonorrea",
    "B. Sífilis primaria",
    "C. Herpes genital",
    "D. Infección por clamydia"
    ],
    respuesta: "B. Sífilis primaria",
    },
    {
    pregunta: "¿Cuál es el tipo de prevención cuyo objetivo es disminuir el avance y las complicaciones de la enfermedad establecida, mediante acciones orientadas a disminuir las secuelas y la discapacidad?",
    opciones: [
    "A. Secundaria",
    "B. Terciaria",
    "C. Cuaternaria",
    "D. Primaria"
    ],
    respuesta: "B. Terciaria",
    },
    
    ];