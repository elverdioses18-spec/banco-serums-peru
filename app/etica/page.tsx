...
useEffect(() => {
  ...
  <Link href="/etica/configurar" className="bg-slate-600 hover:bg-slate-500 text-white px-3 py-3 rounded-xl font-bold text-sm">
    ⬅️ Volver
  </Link>
  ...
  <Link href="/etica/configurar" className="bg-purple-600 hover:bg-purple-500 text-white px-3 py-3 rounded-xl font-bold text-sm">
    🎲 Nuevo examen
  </Link>
</div>
...
>
  ...
  <div className="flex-1 bg-slate-900 text-white p-6">
      ...
      <button
  onClick={() => window.location.href = "/etica/configurar"}
         ...
         onClick={async () => {
          await guardarFalladas(preguntasTema, respuestas);
            ...
            const nuevoAvance = Math.min(
              ...
              preguntasEtica.length
            );
          
            const porcentajeAvance = Math.round(
              (nuevoAvance / preguntasEtica.length) * 100
            );
